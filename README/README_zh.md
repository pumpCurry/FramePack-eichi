...
**FramePack-eichi v1.9.5.3** 強化 LoRA 快取再利用、提升穩定性並增強翻譯功能。

- **強化 LoRA 快取再利用**：增強 FP8/LoRA 最佳化字典資料的磁碟快取功能。
  - 加強並重新命名 1.9.5.2 新增的「重啟時再利用 LoRA 設定」功能。
  - 新增選項，可在生成過程中連續使用最佳化字典並保留於記憶體，不再每次生成後釋放。
    - 在顯示記憶體較少的 GPU 環境中可能無法使用，但避免重複載入可大幅提升生成速度。
- **提高穩定性**：改善作業環境初始化與串流保護，加強啟動時的錯誤防範。
- **保留生成結果**：修正無法保留上次生成結果的錯誤。
- **增強操作日誌**：在載入耗時的 LoRA 快取時顯示進度條等提示。
- **翻譯功能增強**：改進翻譯鍵的一致性。
  - 即使其他語言的 JSON 置於 locales 目錄，也可預期支援該語言的定義檔。
  - 新增翻譯檢查測試檔案。
