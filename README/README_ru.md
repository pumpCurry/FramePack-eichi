...
## 🌟 Улучшения функций (v1.9.5.3)

**FramePack-eichi v1.9.5.3** предлагает усиленное повторное использование кеша LoRA, повышенную стабильность и улучшенную систему переводов.

- **Усиленное повторное использование кеша LoRA**: улучшено кеширование на диск FP8/LoRA-оптимизированных словарей.
  - Усилена и переименована функция «повторное использование настроек LoRA при перезапуске», добавленная в 1.9.5.2.
  - Добавлена опция удерживать оптимизированные словари в памяти и использовать их последовательно без повторного сброса.
    - Может быть недоступно на GPU с небольшим объёмом памяти, но исключает повторные загрузки и значительно ускоряет генерацию.
- **Повышение стабильности**: улучшена инициализация контекста заданий и защита потоков, добавлены меры против ошибок при запуске.
- **Сохранение результата генерации**: исправлена ошибка, из-за которой предыдущий результат терялся.
- **Расширенные журналы**: добавлен индикатор прогресса и другие сообщения при длительной загрузке кеша LoRA.
- **Улучшение перевода**: повышена согласованность ключей перевода.
  - Подготовлена поддержка файлов JSON для других языков, размещенных в каталоге `locales`.
  - Добавлен тестовый файл для проверки переводов.
