...
## 🌟 Улучшения функций (v1.9.5.2)

**FramePack-eichi v1.9.5** сосредоточен на улучшении обратной связи при запуске и расширении управления очередями, чтобы сделать работу ещё удобнее.

### 🚀 Основные нововведения

- **Улучшенные сообщения запуска со спиннером**: во время загрузки модулей отображается индикатор и галочка завершения, чтобы приложение не выглядело зависшим
- **Расширенные очереди изображений и управление пакетами**: можно задавать количество повторов для каждого исходного изображения
  - Число генерируемых кадров теперь рассчитывается как «количество изображений × размер пакета», лимит в 100 снят
- **Очередь и пакетная обработка эталонных изображений**: эталонные изображения можно ставить в очередь с собственным размером пакета
  - Для каждой записи очереди изображений возможно последовательное применение нескольких эталонов
- **Отображение времени прогресса**: показываются прошедшее время и оценка оставшегося времени генерации
- **Опция сохранения входных изображений**: автоматически сохраняет использованные входные изображения в указанную папку
  - Изображения, полученные из буфера обмена, больше не пропадают
- **Настройки вывода журнала**: можно включить сохранение консольных логов, указать и открыть папку для журналов прямо из интерфейса
- **Управление и сохранение избранных настроек**: конфигурации интерфейса можно сохранять как избранные и использовать повторно
- **Кэш подсказок и переключаемый кэш состояния LoRA**: результаты анализа подсказок сохраняются на диск, поддерживается переключение кэша состояния LoRA
- **Поддержка высокого разрешения**: генерация до 2K (2160) разрешения
  - Добавлен режим кадрирования, выравнивающий по длинной стороне с учётом полей при сверке с эталонными изображениями
- **Кнопка «Остановить после текущего»**: прекращает обработку после завершения текущего изображения (интеграция F1/UI)
- **Кнопка «Остановить после шага»**: прекращает обработку после завершения текущего кадра (интеграция F1/UI)
- **Предпросмотр и отображение исходного размера**: возможность просматривать результат в отдельном окне и в модальном окне исходного разрешения
