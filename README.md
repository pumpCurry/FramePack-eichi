...
**FramePack-eichi v1.9.5.1～1.9.5.6**では、Oichiの機能増強を中心に、キャッシュシステムの完成、OOM問題の全面修正、安全性監査による25件の問題修正を行いました。

### 🆕 v1.9.5.5～1.9.5.6 の主な変更
- **キャッシュシステム完成**: LoRA/プロンプトキャッシュの `.pt`/`.safetensors` デュアルフォーマット対応、FP8との同時使用、キャッシュ管理UIパネル
- **OOM問題全面修正**: state_dict二重保持解消、プロンプトキャッシュメモリ蓄積防止、sampling後のGPUテンソル明示解放
- **安全性監査(v1.9.5.6)**: プロンプトキャッシュ汚染、二重起動レース、データ漏洩防止など25件修正
- **3プログラム統一**: スピナー付きCUI起動、5色フェーズ進捗バー、ブラウザ再接続バグ修正を3プログラム全てに適用
- 詳細は [更新履歴](README/README_changelog.md) を参照
...
- **LoRA読込のメモリ保護と高速化**: LoRA設定が変わらない場合は事前アンロードを省略し2回目以降の生成前のロード時間を大幅に削減
  ...
  - `.pt`読み込みをメモリマップ＋`weights_only`化して安全性を高めた
...
- **LoRAワークフロー強化**: ディスク／メモリキャッシュ再利用、メモリ保護、`.pt`のメモリマップ読込、自動補正する設定で安定性を向上
